...
public class MarkerServiceImpl implements MarkerService
{
    ...
    @Override
    public org.springframework.data.domain.Page<MarkerResponse> listVisibleMarkers(Long userId, com.project.gis.dto.MarkerFilter filter, Pageable pageable)
    {
        Specification<Marker> spec = Specification.where(com.project.gis.repository.MarkerSpecification.visibleForUser(userId))
                .and(com.project.gis.repository.MarkerSpecification.byFilter(filter));

        Page<Marker> page = markerRepository.findAll(spec, pageable);

        List<MarkerResponse> content = page.getContent().stream().map(this::toResponse).toList();
    ...
    private MarkerResponse toResponse(Marker m)
    {
        ...
        MarkerResponse.CreatedBy cb = new MarkerResponse.CreatedBy();
        if (m.getCreator() != null) {
            cb.setUserId(m.getCreator().getId());
            cb.setUsername(m.getCreator().getUsername());
        }
        r.setCreatedBy(cb);
